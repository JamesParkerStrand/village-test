...
function checkClicking() {
    ...
    const boxs = aabbFromCenter(center, [5, 5, 5]);

    var citizenMesh = noa.entities.getMeshData(citizen).mesh;

    const box = BABYLON.MeshBuilder.CreateBox("box", { size: 2 }, scene);
    box.position = new BABYLON.Vector3(charPos[0],charPos[1]+1,charPos[2]);
    box.isPickable = true;
    

    const origin = new BABYLON.Vector3(center[0],center[1]+1,center[2]);

    const camDir = noa.camera.getDirection();

// target point you want to aim at

    // normalized direction vector
    const dir = new BABYLON.Vector3(camDir[0],camDir[1],camDir[2]);
    ...
    // cast the ray against the whole scene (or give a predicate)
    const pick = scene.pickWithRay(ray);

    if (pick && pick.hit) {
    console.log("World ray hit the box!");
    console.log("Picked point:", pick.pickedPoint);
    console.log("Distance from origin:", BABYLON.Vector3.Distance(origin, pick.pickedPoint));
    } else {
    ...
    const hits = noa.entities.getEntitiesInAABB(boxs);
    /* 
    if(hits[2] != null) {
        doText();
    }
